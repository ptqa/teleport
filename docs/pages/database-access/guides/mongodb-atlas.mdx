...
teleport db start \
  ...
  --ca-cert=/path/to/letsencrypt/root-ca
  --labels=env=dev
```
...
The `--host` flag value doesn't matter in this case since you'll only use the
CA certificate which this command will output to `mongo.cas` file. You can
discard the other `mongo.crt` file.
